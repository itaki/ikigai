...
class ADS1115:
    ...
    def read_pins(self, data_rate):
        ...
        while not self._stop_thread.is_set():
            ...
            if not self.is_initialized and len(self.readings[0]) >= self.initial_readings:
                self.is_initialized = True
                logger.info(f"✅ ADS1115 {self.label} initialized with {len(self.readings[0])} readings")
                break
